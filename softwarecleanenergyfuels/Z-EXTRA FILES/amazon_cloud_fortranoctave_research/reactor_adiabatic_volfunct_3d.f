...
C   cell: 203-945-8606
      ...
      program reactor_adiabatic_volfunct_3d
      EXTERNAL RES, JACDUM
      INTEGER NEQ,INFO(15),IDID,LRW,IWORK(100000),LIW,IPAR(1000),i
      DOUBLE PRECISION T, TOUT, RTOL, ATOL,
     * RWORK(100000), RPAR(1000)

      DOUBLE PRECISION,ALLOCATABLE, dimension(:) :: Y, YPRIME

      INTEGER KK, II, EE, nP, nT, nV
      CHARACTER (LEN=16) fuel_species(1)
      INTEGER fuel_ratio(1)
      CHARACTER (LEN=16) oxidant_species, dilutant_species
      DOUBLE PRECISION dilutant_ratio
      INTEGER c_order, h_order, o_order, j
      INTEGER c_tot, h_tot, o_tot, A, Yfuel
      INTEGER,ALLOCATABLE, dimension(:,:) :: COMP
      DOUBLE PRECISION amount_oxidant
      DOUBLE PRECISION,ALLOCATABLE, dimension(:) :: Y0, WT

      DOUBLE PRECISION P0atm, PATM, P0, Temp, Phi
      DOUBLE PRECISION f, R, omega, alpha, PI, WM0, v0, RU
C     number of time steps
      INTEGER Tsteps
C     length of time span in seconds, and change-in-time/step
      DOUBLE PRECISION Tspan, Deltatime

      CALL GETPATM (PATM)
C     system conditions
      P0atm = 1
C     1 atm = 1 013 250 erg / (centimeter^3)
      P0 = P0atm*PATM
...
C     angular velocity
      ...
      CALL FINDELEMENT (EE, "C  ",c_order)
      CALL FINDELEMENT (EE, "H  ",h_order)
      CALL FINDELEMENT (EE, "O  ",o_order)
      c_tot=0
      h_tot=0
      o_tot=0
      CALL GETCOMPOSITION(EE,KK,COMP)
      do 20 j = 1, SIZE(fuel_species)
        CALL FINDSPECIES(KK, fuel_species(1),A)
        c_tot = c_tot + fuel_ratio(j) * comp(c_order,A)
        h_tot = h_tot + fuel_ratio(j) * comp(h_order,A)
        o_tot = o_tot + fuel_ratio(j) * comp(o_order,A)
  20  continue

C     calculate stoich amount of oxidant
      amount_oxidant=h_tot/4.0 + c_tot - o_tot/2.0
C     initial mass fractions
      do 400 j = 1, SIZE(fuel_species)
        CALL FINDSPECIES(KK, fuel_species(j),A)
        Y0(A) = WT(A)*fuel_ratio(j)
  400 continue
      CALL FINDSPECIES(KK, oxidant_species,A)
      Y0(A) = WT(A)*amount_oxidant/Phi
      CALL FINDSPECIES(KK, dilutant_species,A)
      Y0(A) = WT(A)*dilutant_ratio*amount_oxidant/Phi
      Y0 = Y0/SUM(Y0)
...
C     IC vectors  into the main IC vector
      Y(1:KK) = Y0
      Y(nP) = P0
      Y(nT) = Temp
      Y(nV) = v0
...
C      Simple initial conditions, mass fraction, pressure
C      temperature, v0 for phi=0.3 temp=500 p0atm=1
C      f=194.3   R=16   phi=0.3   
C      Y(1:56) = 0.0
C      Y(4) =  2.2899e-01
C      Y(14) = 1.7221e-02
C      Y(48) = 7.5378e-01
...
C      Y(55) = 5.0000e+02
C      Y(56) = 1.4417e+03
C      have to add v0,omega,alpha to Res function if want to 
C      do simple initial conditions
C      v0 =  1441.7
C      omega =  1220.8
C      alpha =  0.88235
      ...
      CALL FINDSPECIES(KK, fuel_species(1),Yfuel)

      DO 10 i = 1,(Tsteps+1)
        CALL DDASSL (RES, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL, ATOL,
     *   IDID, RWORK, LRW, IWORK, LIW, RPAR, IPAR, JACDUM)
        PRINT *, T, Y(nT), Y(Yfuel)
        TOUT = TOUT+Deltatime
  10  continue


      STOP
      end program reactor_adiabatic_volfunct_3d
      ...
      INTEGER KK, II, nP, nT, nV
      DOUBLE PRECISION  T, RPAR(1000), P, Temp, RU 
      DOUBLE PRECISION v0, alpha, omega
      DOUBLE PRECISION WM, RHO, dvdt, CVM
C     vectors NEQ long
      DOUBLE PRECISION,dimension(56) :: Y, YDOT, DELTA
C     vectors KK long
      DOUBLE PRECISION,dimension(53) :: CV, U, C,
     *   WDOT, YMassfrac, WT
      ...
      WM = 1/SUM(YMassfrac/WT)

      RHO = P*WM/(RU*Temp)

      CVM = DOT_PRODUCT(CV,YMassfrac)

      dvdt = -v0*omega*alpha*sin(omega*T)/(1+alpha)

      DELTA(nV) = YDOT(nV) - dvdt

      C = (YMassfrac/WT)*RHO

      CALL GETWC(KK,II,Temp,C,WDOT)

      DELTA(1:KK) = WDOT*WT/RHO - YDOT(1:KK)

      DELTA(nT) = YDOT(nT) + (P*dvdt + sum(U*WDOT*WT)/RHO)/CVM

      DELTA(nP) = YDOT(nP) - 
     *   (T*RU*sum(YDOT(1:KK)/WT)+(RU/WM)*YDOT(nT)-P*dvdt)*RHO
