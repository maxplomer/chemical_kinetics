Cexample how to declare then allocate
C      INTEGER,ALLOCATABLE, dimension(:,:) :: COMP
C      ALLOCATE(COMP(EE,KK))
C      CALL GETCOMPOSITION (EE,KK,COMP)

CCCCCCCCCCCC program to validate WDOT 
C      program odesolver
C      INTEGER KK II i
C      DOUBLE PRECISION T, Y(53), WDOT(53)
C      KK = 53
C      II = 325
C      T = 1000
C      Y(1:53) = 1.0
C      CALL GETWC (KK,II,T,Y,WDOT)
C      do 10 i = 1, KK
C        PRINT *, WDOT(i)
C  10  continue
C      end program odesolver


CCCCCCCCCCCC program to validate CV 
C      program odesolver
C      INTEGER KK i
C      DOUBLE PRECISION T, CV(53)
C      KK = 53
C      T = 1000
C      CALL GETCV (KK,T,CV)
C      do 10 i = 1, KK
C        PRINT *, CV(i)
C  10  continue
C      end program odesolver

CCCCCCCCCCCC program to validate U
C      program odesolver
C      INTEGER KK i
C      DOUBLE PRECISION T, U(53)
C      KK = 53
C      T = 1000
C      CALL GETU (KK,T,U)
C      do 10 i = 1, KK
...
C use SIZE() to get length of array if need
      ...
      DOUBLE PRECISION RU
C system conditions
      DOUBLE PRECISION P0atm = 1
      DOUBLE PRECISION P0 = P0atm*1013250
      DOUBLE PRECISION Temp = 900
      ...
      ALLOCATE(X0(KK))
      ALLOCATE(Y(KK+1))
      ALLOCATE(IWORK(20+NEQ(1)))
      ALLOCATE(RWORK(22 +  9*NEQ(1) + NEQ(1)**2))
C  LIW/LRW  is     Declared length of IWORK/RWORK
      LIW = 20+NEQ(1)
      LRW = 22 +  9*NEQ(1) + NEQ(1)**2
...
C calculate stoich amount of oxidant
      amount_oxidant=h_tot/4 + c_tot - o_tot/2

C initial mol fractions
      do 400 j = 1, SIZE(fuel_species)
        CALL FINDSPECIES(KK, fuel_species(1),A)
        X0(A) = fuel_ratio(j)
  400 continue
      CALL FINDSPECIES(KK, oxidant_species,A)
      X0(A) = amount_oxidant/Phi
      CALL FINDSPECIES(KK, dilutant_species,A)
      X0(A) = dilutant_ratio*amount_oxidant/Phi
      X0 = X0/SUM(X0)

C IC vectors  into the main IC vector
C convert mol frac to mol concentration
      Y(1:KK) = X0*P0/(RU*Temp)
      Y(KK+1) = Temp

      
      T = 0.D0
      TOUT = 0.0001
C ITOL = 1 if ATOL not array, 2 if array
      ITOL = 1
      RTOL(1) = 0.001
      ATOL(1) = 0.001
C if ITASK = 1   then performs computation, useless variable?
      ITASK = 1
C if ISTATE = 1   This is the first call for a problem.
      ISTATE = 1
C if IOPT = 0     No optional inputs are used
      IOPT = 0
C if MF = 22  Stiff method, internally generated full Jacobian.
      MF = 22                
