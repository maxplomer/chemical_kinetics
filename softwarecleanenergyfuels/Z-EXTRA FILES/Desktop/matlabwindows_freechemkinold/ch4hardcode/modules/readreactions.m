...
while 1
    tline = fgetl(fid);
    eqn1=strread(tline,'%s','delimiter',' ');%eqn1 is tline delimiter ' ', get first line info
    ...
    while 1 %if there is a = <=> <= or => then reactionnum=reaction+1;, couldn't convert this to a switch statement because using findstr function
        if(findstr(eqn, '<=>'))
            ...
            lookforPdependence();%look for pressure dependence, remove (+___) then add to M array
            lookforcoeffients();
            break;
        end
        ...
        if(findstr(eqn, '=>'))
            ...
            lookforPdependence();%look for pressure dependence, remove then add to M array
            lookforcoeffients();
            break;
        end
        if(findstr(eqn, '='))
