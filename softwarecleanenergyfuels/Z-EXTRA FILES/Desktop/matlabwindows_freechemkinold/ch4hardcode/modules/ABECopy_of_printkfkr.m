...
for i=1:n
    ...
    if (isempty(M{i}) || strcmpi(M{i},'M'))%if no pressure dependence, this is for normal arrhenius or 3rd body
        fprintf(fid, 'kf=A(%g)*T^B(%g)*exp(-E(%g)*41840000/(RU*T))',i,i,i);
        if (strcmpi(M{i},'M'))
            fprintf(fid, '*(sumY');
    ...
    fprintf(fid, ';\n');
    fprintf(fid, 'kinf=A(%g)*T^B(%g)*exp(-E(%g)*41840000/(RU*T));\n',i,i,i);
    ...
    if (strcmpi(M{i},'(+M)'))
        ...
        Pspec=strread(M{i},'%s','delimiter','(');
        Pspec=strread(Pspec,'%s','delimiter',')');
        Pspec=strread(Pspec,'%s','delimiter','+');
        I_species=find(strcmp(species,Pspec));
        fprintf(fid, 'Pr=(k0*Y(%g))/kinf;\n',I_species);
    end
    ...
    if (~isempty(Troe{i}))
        ...
        fprintf(fid, 'd = 0.14;\n');
        fprintf(fid, 'F=exp(log(Fcent)*(1+(  (log10(Pr)+c)/(n-d*(log10(Pr)+c))  )^2)^-1);\n');
    else
        fprintf(fid, 'F=1;\n');
    end
