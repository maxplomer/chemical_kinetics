      program odesolver
      EXTERNAL RES, JACDUM
      INTEGER NEQ, INFO(15), IDID, LRW, LIW, IWORK(LIW), IPAR, i
      DOUBLE PRECISION T, Y(3), YPRIME(3), TOUT, RTOL, ATOL,
     *   RWORK(76), RPAR
C allocate:   Y(NEQ), YPRIME(NEQ), IWORK(LIW), INFO(N), RWORK(LRW)
...
C **** Is this the first call for this problem ...Yes
      INFO(1) = 0
C **** both error tolerances RTOL, ATOL scalars(not array)...Yes
      INFO(2) = 0
C **** Do you want the solution only at
C      TOUT (and not at the next intermediate step) ...Yes
      INFO(3) = 0
C **** Can the integration be carried out without any
C      restrictions on the independent variable T ...Yes
      INFO(4) = 0
C **** Do you want the code to evaluate the partial
C      derivatives automatically by numerical differences ...Yes
      INFO(5) = 0
...
C       on its own maximum stepsize? ...Yes
      INFO(7) = 0
C ****  Do you want the code to define
C        its own initial stepsize? ...Yes
      INFO(8) = 0
C ****  Do you want the maximum order to
C       default to 5? ...Yes
      INFO(9) = 0
C ****  Do you want the code to solve the problem without
C       invoking any special nonnegativity constraints? ...Yes
      INFO(10) = 0
C ****   Are the initial T, Y, YPRIME consistent? ...Yes
      INFO(11) = 0

      RTOL = 1.D-6
      ATOL = 1.D-6

C  IDID is indicator reporting what the code did

C  MAXORD is default 5
C  LRW is length of the RWORK array.
C  LRW >= 40+(MAXORD+4)*NEQ+NEQ**2
C  for the full (dense) JACOBIAN case (when INFO(6)=0)
C  40 + (5+4)*3 + 3*3 = 76
      LRW = 76

C  LIW is length of the IWORK array.
C  LIW >= 20+NEQ
C  20 +3 = 23
      LIW = 23
  ...
  10  continue
      ...
      INTEGER IRES, IPAR
      DOUBLE PRECISION  T, Y(3), YDOT(3), DELTA(3), RPAR
      DELTA(1)=-YDOT(1) - Y(1)
      DELTA(2)= Y(1) - Y(2) - YDOT(2)
      DELTA(3)= Y(2) - YDOT(3)
      RETURN
      END
