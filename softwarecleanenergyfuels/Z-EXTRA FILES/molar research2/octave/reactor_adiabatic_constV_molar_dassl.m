function reactor_adiabatic_constV_molar_dassl
...
% setup and run the octave-dassl integration solver
...
ylabel('Temperature (K)');
    ...
    %
    function res = ignfun(y,dydt,t)%mol concentration,dcdt,time
        KK=getKK;
        NV=KK+2;
        nY = 1:KK;
        nT = KK+1;
        ...
        res(nT)=-dydt(nT) - (dot(u,wdot)/ctot)/cvm;
        
        %Change in Pressure per time, dP/dt 
        res(nP)= -dydt(nP) + (RU*dydt(nT))*ctot;
