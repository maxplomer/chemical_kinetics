      program reactor_perfectlystirred
      EXTERNAL RES, JACDUM
     ...
     * RWORK(100000), RPAR(1000)
      ...
      DOUBLE PRECISION,ALLOCATABLE, dimension(:) :: Y0, WT, h0

      DOUBLE PRECISION P0atm, PATM, P, Temp, Phi, tau
...
C      temperature for ch4 phi=0.5 temp=1200 p0atm=1 tau=1
C      Y(1:54) = 0.0
C      Y(4) =  2.26395764862761e-01
C      Y(14) = 2.83756344142613e-02
C      Y(48) = 7.45228600722978e-01
C      Y(54) = 1.20000000000000e+03
  ...
  10  continue
      ...
      STOP
      end program reactor_perfectlystirred
      ...
      INTEGER KK, II, nT
      DOUBLE PRECISION  T, RPAR(1000), P0, Temp, RU
      DOUBLE PRECISION WM, RHO, CPM, tau 
C     vectors NEQ long
      DOUBLE PRECISION,dimension(54) :: Y, YDOT, DELTA
C     vectors KK long
      DOUBLE PRECISION,dimension(53) :: CP, H, C,
     *   WDOT, YMassfrac, WT, h0, Y0
      ...
      CPM = DOT_PRODUCT(CP,YMassfrac)

      C = (YMassfrac/WT)*RHO

      CALL GETWC(KK,II,Temp,C,WDOT)

      DELTA(1:KK) = -YDOT(1:KK) + (Y0-YMassfrac)/tau + 
     *   WDOT*WT/RHO

      DELTA(nT) = -YDOT(nT) + 
     *   sum(YMassfrac*(h0-H))/(tau*CPM) - 
     *   sum(H*WDOT*WT)/(RHO*CPM)
